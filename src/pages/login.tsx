...
const LoginPage = ({ csrfToken }) => {
  ...
  if (status === 'authenticated' && data.user.email !== null) {
    router.push('/');
  }
  ...
  return (
    <div className='bg-white sm:bg-gray-200 min-h-screen w-screen flex flex-col justify-center items-center'>
      <div className='bg-white shadow-none sm:shadow-lg px-8 sm:px-12 w-full xs:w-full sm:w-8/12 md:w-7/12 lg:w-7/12 xl:w-2/6 h-screen sm:h-auto py-8'>
        <div className='text-center w-full font-bold text-3xl text-gray-600 p-4'>LOGIN</div>
        ...
        <form method='post' action='/api/auth/callback/credentials'>
          ...
          <div className='flex flex-col gap-4 px-0 py-4'>
            <div>
              ...
              <input name='email' className='py-2 pl-10 border border-gray-200 w-full' placeholder='Email address' type='email' required />
            </div>
            <div>
              ...
              <input name='password' className='py-2 pl-10 border border-gray-200 w-full' placeholder='Password' type='password' required />
            </div>
      ...
      <div className='p-4'>© CHAICOPADILLA 2021</div>
    </div>
  );
};
